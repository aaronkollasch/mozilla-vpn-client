...
void LocalSocketController::activate(
  ...
  logger.debug() << "LocalSocketController activated";
  activateNext();
}
...
void LocalSocketController::peerConnected(const QString& pubkey) {
...
void LocalSocketController::parseCommand(const QByteArray& command) {
  ...
  if (type == "connected") {
    QString pubkey = obj.value("publicKey").toString();
    if (!pubkey.isEmpty()) {
      peerConnected(pubkey);
      return;
    }
    logger.debug() << "connected without public key";

    // We don't expect to get here, something is mismatched...
    // TODO: Remove this.
